            ...
            try {
                ...
                if (state.error().isPresent()) {
                    ...
                    if (state.error().isPresent()) {
                        ...
                        Logger.info("Fixed blockstate to: " + state.getOrThrow(err -> new IllegalStateException("Failed to decode blockstate: " + err)));
                        forceResave[0] |= true;
                        return new StateEntry(id, state.getOrThrow(err -> new IllegalStateException("Failed to decode blockstate: " + err)));
                    }
                } else {
                    return new StateEntry(id, state.getOrThrow(err -> new IllegalStateException("Missing state: " + err)));
                }
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
